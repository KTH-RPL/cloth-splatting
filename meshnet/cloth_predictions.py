...
#from manipulation.dynamics_evaluation import predict
...
def get_predictions(simulator, ds, features, nsteps, device, input_sequence_length, dt, data_path=None, get_from_path=False):
    if get_from_path:
...
def predict(simulator, device, FLAGS, exp_name):
    ...
    with torch.no_grad():
        ...
        num_trajs = len(ds.dataset._data)
        num_trajs = 3
        for t in range(num_trajs):
            ...
            gif_file = os.path.join(gif_path, f'rollout_{t}.gif')
...
def main(_):
    ...
    mode_path_original = FLAGS.model_path
    original_data_path = FLAGS.data_path
    
    for o in ['TOWEL', 'TSHIRT']:
        FLAGS.object = o
        ...
        for i in range(1, 6, 1):
            ...
            model_path = os.path.join(mode_path_original, exp_name)
            # TODO: change this
