...
def build_depth_from_pointcloud(pointcloud, matrix_world_to_camera, imsize, fov=80):
    height, width = imsize
    ...
    K = intrinsic_from_fov(height, width, fov)  # the fov is 90 degrees
...
def intrinsic_from_fov(height, width, fov=80):
    """
...
def get_world_coords(rgb, depth, env, particle_pos=None, fov=80):
...
def project_to_image(matrix_world_to_camera, world_coordinate, height=360, width=360, fov=80):
...
def get_world_coor_from_image(u, v, image_size, matrix_world_to_camera, all_depth, fov):
